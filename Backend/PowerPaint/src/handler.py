...
def handler(job):
    ...
    # Run the inference
    output_images, _ = controller.infer(*controller_inputs)

    output_img_base64 = image_to_base64(output_images[0])
    # Return the first (and only) output image
    return {"image" : output_img_base64}
